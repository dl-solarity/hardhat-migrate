  ...
  describe("getContractDeployParams()", () => {
    ...
    context("pure ethers", () => {
      ...
      it("should get abi", async () => {
        const abi = adapter.getContractDeployParams(ContractWithConstructor).abi;

        expect(abi).to.deep.equal(Interface.from(contractWithConstructorABI));
      });

      it("should get bytecode", async () => {
        const bytecode = adapter.getContractDeployParams(ContractWithConstructor).bytecode;
    ...
    context("with typechain", () => {
      ...
      it("should get abi", async () => {
        const abi = adapter.getContractDeployParams(ContractWithConstructorArguments__factory).abi;

        expect(abi).to.deep.equal(Interface.from(contractWithConstructorABI));
      });

      it("should get bytecode", async () => {
        const bytecode = adapter.getContractDeployParams(ContractWithConstructorArguments__factory).bytecode;
